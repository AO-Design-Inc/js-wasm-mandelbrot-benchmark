...
//defined for performance measurement, depended on by worker that loads them
let start,
    end = 0;
...
function init_wasm(wasm_path, importObj) {
    ...
    if (WebAssembly.instantiateStreaming) {
        ...
        return fetch(wasm_path, { cache: 'force-cache' })
...
function returnSharedBufferjs(
    ...
    return init_wasm(
        './benchmarks/simd-assemblyscript-singlethreaded/build/mandel_final.wasm',
        importObj
    )
        .then((result) => {
