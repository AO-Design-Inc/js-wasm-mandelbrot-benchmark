/* globals start, end, returnNaivejs, returnImprovedjs, returnSharedBufferjs, run_wrapper, computeAndDrawMandel */
...
onmessage = async function (e) {
    ...
    switch (e.data[0]) {
        ...
        case 'multijs':
            ...
            imageData = await returnSharedBufferjs(
                START_X_TOTAL,
                START_Y_TOTAL,
                e.data[1],
                e.data[2],
        ...
        case 's-rustwasm':
            if (!scriptImported) {
                self.importScripts(
                    './benchmarks/singlethreaded-rustwasm/Mandelbrot.js'
                );
                scriptImported = true;
            }
            imageData = await run_wrapper(
                START_X_TOTAL,
                START_Y_TOTAL,
                e.data[1],
                e.data[2],
        ...
        case 'assemblyscript-multithreaded':
            if (!scriptImported) {
                self.importScripts(
                    '/benchmarks/assemblyscript-multithreaded/main.js'
                );
                scriptImported = true;
            }
            imageData = await computeAndDrawMandel(
                START_X_TOTAL,
                START_Y_TOTAL,
                e.data[1],
                e.data[2],
        ...
        case 'assemblyscript-optimized-single':
            if (!scriptImported) {
                self.importScripts(
                    '/benchmarks/optimized-assemblyscript-singlethreaded/main.js'
                );
                scriptImported = true;
            }
            imageData = await returnSharedBufferjs(
        ...
        case 'simd-assemblyscript-single':
            if (!scriptImported) {
                self.importScripts(
                    './benchmarks/simd-assemblyscript-singlethreaded/main.js'
                );
                scriptImported = true;
            }
            imageData = await returnSharedBufferjs(
